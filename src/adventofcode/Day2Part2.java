...
public class Day2Part2 {
	...
	public static void main(String args[]) {
		String inputData = "157	564	120	495	194	520	510	618	244	443	471	473	612	149	506	138\r\n"
		...
		System.out.println(computeCheckSum(inputData));
	}

	public static int computeCheckSum(String inputData) {
		if (inputData == null) {
			return -1;
		}
		...
		for (int i = 0; i < rows.length; i++) {
			...
			for (int j = 0; j < columns.length; j++) {
				int currentColumn = Integer.valueOf(columns[j]);
				for (int k = 0; k < columns.length; k++) {
					int cursorColumn = Integer.valueOf(columns[k]);
					if(k != j)
					{
						if(currentColumn % cursorColumn == 0) {
							chkSum = chkSum + (currentColumn/cursorColumn);
						}
					}
				}
				
			}
			chkSum = chkSum + (max- min);
		}
		return chkSum;
	}
